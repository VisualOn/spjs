...
(function (spjs) {
    ...
    spjs.utility = { version: 1.183, versionDate: "May 13, 2014" };
    ...
    function doResizeDialog() {
        ...
        if (dlgContent.attr('DragBehavior') !== null && dlgContent.css('left') !== '10px') {
            ...
            cssObj = { position: 'absolute', height: formH + 'px' };
            ...
            dlgInnerFrame.css({ height: (formH - 32) + 'px' });
        }
    }
    ...
    function spjs_UpdateList(listName, listBaseUrl, newFieldsObjArr, updFieldsObjArr, deleteFieldsArr) {
        ...
        if (deleteFieldsArr !== undefined) {
            $.each(deleteFieldsArr, function (i, fin) {
    ...
    function getUserInfo_v2(loginOrUserID, customProp) {
        ...
        res = spjs_QueryItems({ 'listName': 'UserInfo', 'listBaseUrl': L_Menu_BaseUrl, 'query': query, 'viewFields': arrOfFields, setRequestHeader: false });
        if (res.count > 0) {
            result.success = true;
            item = res.items[0];
    ...
    function spjs_getItemByID(argObj) {
        ...
        qRes = spjs_QueryItems({ 'listName': argObj.listName, 'listBaseUrl': argObj.listBaseUrl, 'query': query, 'viewFields': argObj.viewFields, 'scope': argObj.scope });
        if (qRes.count === 0) {
            return null;
        } else {
            return qRes.items[0];
    ...
    function spjs_QueryItems(argObj) {
        ...
        content = spjs_wrapQueryContent({ 'listName': argObj.listName, 'query': argObj.query, 'viewName': argObj.viewName, 'viewFields': argObj.viewFields, 'rowLimit': argObj.rowLimit, 'pagingInfo': argObj.pagingInfo, 'scope': argObj.scope });
    ...
    function spjs_wrapQueryContent(paramObj) {
        ...
        if (paramObj.scope !== undefined) {
            result.push('<ViewAttributes Scope="' + paramObj.scope + '" />');
        }
    ...
    function setFieldValue(fin, newVal, onLoad) {
        ...
        switch (fieldType) {
            ...
            case 'SPFieldChoice':
                if (thisField.find('input:radio').length > 0) {
                    thisField.find('input:radio').prop("checked", false);
        ...
        if (thisField.find("td.ms-formbody").length > 1) {
            thisField.find("td.ms-formbody:last").html(newVal);
        }
        return null;
    }
    ...
    */
    function spjs_addItem(argObj) {
        var qRes = spjs_UpdateListItem({ 'action': 'new', 'listName': argObj.listName, 'listBaseUrl': argObj.listBaseUrl, 'data': argObj.data });
        return qRes;
    }
    ...
    */
    function spjs_deleteItem(argObj) {
        var qRes = spjs_UpdateListItem({ 'action': 'delete', 'listName': argObj.listName, 'listBaseUrl': argObj.listBaseUrl, 'id': argObj.id, 'docFullUrl': argObj.docFullUrl });
        return qRes;
    }
    ...
    */
    function spjs_updateItem(argObj) {
        var qRes = spjs_UpdateListItem({ 'action': 'update', 'listName': argObj.listName, 'listBaseUrl': argObj.listBaseUrl, 'id': argObj.id, 'data': argObj.data });
        return qRes;
    }
    ...
    spjs.getFieldValue = getFieldValue;
    spjs.setFieldValue = setFieldValue;


    spjs.getField = function(fin) {
        if (!fields)
            fields = init_fields_v2();
    ...
    spjs.addList = spjs_AddList;
    spjs.queryItems = spjs_QueryItems;
    spjs.updateList = spjs_UpdateList;
    spjs.updateItem = spjs_UpdateListItem;
    spjs.addItem = spjs_addItem;
    spjs.deleteItem = spjs_deleteItem;
    spjs.getItemByID = spjs_getItemByID;
    spjs.updateItem = spjs_updateItem;
    spjs.wrapQueryContent = spjs_wrapQueryContent;
    spjs.wrapSoapRequest = spjs_wrapSoapRequest;
    spjs.doResizeDialog = doResizeDialog;
    spjs.getUserInfo = getUserInfo_v2;
})(spjs || (spjs = {}));
